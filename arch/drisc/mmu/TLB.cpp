...
Result TLB::lookup(RPAddr const processId, RMAddr const vAddr, int const d$line, bool *r, bool *w, RMAddr *pAddr){
	...
	Line *line = find(processId, vAddr);
	if(line == NULL){
		Line dest = m_tables[0]->pickDestination();
		...
		dest.pending.processId = processId;
		dest.pending.vAddr = vAddr;
		dest.pending.present = false;
		dest.pending.locked = true;
		dest.pending.d$lineId = d$line;
	}
}

Line* TLB::find(RPAddr processId, RMAddr vAddr){
	//MLDTODO Filter on line type?
	Line *discoveredEntry = NULL; //MLDTODO Remove after debugging

	for(Table *table : m_tables){
		RMAddr truncatedAddr = vAddr.truncateLsb(table->getVAddrWidth());
		Line *line = *table->lookup(processId, truncatedAddr);
		if(line != NULL){
...
//Line* TLB::lookup(RPAddr pid, RMAddr addr){ //MLDTODO Rename to processId and vAddr
//	assert(addr.m_width == RMAddr::VirtWidth);
//
//	for(Table *table : m_tables){
//		RMAddr tableAddr = addr.truncateLsb(table->getOffsetWidth());
//		Line *res = table->lookup(pid, tableAddr);
//
//		if(res != NULL){
//			return res;
//		}
