...
static const struct argp_option mgsim_options[] =
{
    ...
    { "debug-mode", 'D', "NUM", 0, "Initial trace / debug mode.", 9 },
...
static error_t mgsim_parse_opt(int key, char *arg, struct argp_state *state)
{
    ...
    {
        char* endptr;
        unsigned int mode = strtoul(arg, &endptr, 0);
        if (*endptr != '\0') {
            throw runtime_error("Error: unable to parse debug mode");
        } else {
            config.m_debugMode = mode;
        }

        config.m_overrides.append("DebugMode", arg);
    }
    break;
