...
const char* b_4k_name(){
	return "Bench read, page: 4k, cached: -";
}


int b_4k_pre(tlbRef_t tlbReference, char quiet){
	// Invalidate the TLB
...
void b_4k_run(sl_place_t destination, result_t* result, char abort, char quiet){
...
sl_def(b_4k,, sl_shparm(result_t*, result), sl_shparm(char, abort), sl_shparm(char, quiet)){
	...
	// Time the read
//	svp_abort();
	int cycles = timedRead_asm(loc, 248);
//	svp_abort();
	if(cycles < 0){
