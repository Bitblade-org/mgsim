...
const char* b_4k_pd_name(){
	return "Bench read, page: 4k, cached: PT, DATA";
}


int b_4k_pd_pre(tlbRef_t tlbReference, char quiet){
...
void b_4k_pd_run(sl_place_t destination, result_t* result, char abort, char quiet){
...
sl_def(b_4k_pd,, sl_shparm(result_t*, result), sl_shparm(char, abort), sl_shparm(char, quiet)){
