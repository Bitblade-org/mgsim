...
const char* b_4k_td_name(){
	return "Bench read, page: 4k, cached: TLB DATA";
}


int b_4k_td_pre(tlbRef_t tlbReference, char quiet){
...
void b_4k_td_run(sl_place_t destination, result_t* result, char abort, char quiet){
...
sl_def(b_4k_td,, sl_shparm(result_t*, result), sl_shparm(char, abort), sl_shparm(char, quiet)){
